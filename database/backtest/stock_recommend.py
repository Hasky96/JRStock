...
def recommend_stocks(data):
    ...
    data['rv'] = ((data['rank_by_ROE'] + data['rank_by_EPS'] + data['rank_by_BPS'] + data['rank_by_PER'])/4).rank(method='dense').astype(int)
    return data.sort_values("rv", ascending=False).loc[:,['ROE','rank_by_ROE', 'EPS','rank_by_EPS', 'BPS','rank_by_BPS', 'PER','rank_by_PER', 'rv']].sort_values('rv')
