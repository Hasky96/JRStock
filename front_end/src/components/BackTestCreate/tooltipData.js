...
const contentDict = {
  ...
  203: {
    info: "이동평균수렴확산지수(MACD)는 기간이 다른 이동평균선 사이의 관계에서 추세변화의 신호를 찾으려는 진동자 지표이다. 신호선은 MACD의 지수 이동 평균이다./nMACD가 신호선을 상향돌파하면 골든크로스로 판단한다.",
    params: {
      3: "단기 이동평균선 기간",
      4: "장기 이동평균선 기간",
      5: "신호선 기간",
  ...
  204: {
    info: "이동평균수렴확산지수(MACD)는 기간이 다른 이동평균선 사이의 관계에서 추세변화의 신호를 찾으려는 진동자 지표이다. 신호선은 MACD의 지수 이동 평균이다./nMACD가 신호선을 하향돌파하면 데드크로스로 판단한다.",
    params: {
      3: "단기 이동평균선 기간",
      4: "장기 이동평균선 기간",
      5: "신호선 기간",
  ...
  205: {
    info: "이동평균수렴확산지수(MACD)는 기간이 다른 이동평균선 사이의 관계에서 추세변화의 신호를 찾으려는 진동자 지표이다. 신호선은 MACD의 지수 이동 평균이다./nMACD가 신호선보다 위에 있다면 정배열로 판단한다.",
    params: {
      3: "단기 이동평균선 기간",
      4: "장기 이동평균선 기간",
      5: "신호선 기간",
  ...
  206: {
    info: "이동평균수렴확산지수(MACD)는 기간이 다른 이동평균선 사이의 관계에서 추세변화의 신호를 찾으려는 진동자 지표이다. 신호선은 MACD의 지수 이동 평균이다./nMACD가 신호선보다 아래에 있다면 역배열로 판단한다.",
    params: {
      3: "단기 이동평균선 기간",
      4: "장기 이동평균선 기간",
      5: "신호선 기간",
  ...
  307: {
    info: "상대적강도지수(RSI)는 미래 주가의 강세 및 약세를 전일 대비(혹은 전주비)의 주가 변화의 비율로 예측하려고 하는 지표이다. RSI는 과거 일정 기간의 가격 상승폭의 합계(분자)에 대한 같은 기간 내의 가격 상승폭과 가격 하락폭의 절대치 합계(분모)의 백분비로 구해진다. 경험적으로 80%이상은 천정으로 매도 영역, 25% 이하는 바닥권으로 매입 영역을 나타낸다./nRSI가 임계치보다 위에 있다면 자산가지 고평가로 판단한다.",
    params: {
      1: "RSI 기간",
      6: "고평가 판단 기준",
    },
  ...
  308: {
    info: "상대적강도지수(RSI)는 미래 주가의 강세 및 약세를 전일 대비(혹은 전주비)의 주가 변화의 비율로 예측하려고 하는 지표이다. RSI는 과거 일정 기간의 가격 상승폭의 합계(분자)에 대한 같은 기간 내의 가격 상승폭과 가격 하락폭의 절대치 합계(분모)의 백분비로 구해진다. 경험적으로 80%이상은 천정으로 매도 영역, 25% 이하는 바닥권으로 매입 영역을 나타낸다./nRSI가 임계치보다 아래에 있다면 자산가지 저평가로 판단한다.",
    params: {
      1: "RSI 기간",
      6: "저평가 판단 기준",
    },
  ...
  407: {
    info: "OBV는 상승한 날의 거래량과 하락한 날의 거래량을 누계적으로 집계, 도표화한 것이다. 거래량은 항상 주가에 선행한다는 것을 전제로 거래량 분석을 통해 주가를 분석하는 기법으로 조셉 그렌빌(Joseph Granville)이 개발한 기법이다. OBV_EMA 는 OBV의 지수이동평균 선이다./nOBV가 OBV_EMA위로 오는 경우 높음으로 판단한다.",
    params: {
      1: "OBV, OBV_EMA 기간",
    },
  },
  408: {
    info: "OBV는 상승한 날의 거래량과 하락한 날의 거래량을 누계적으로 집계, 도표화한 것이다. 거래량은 항상 주가에 선행한다는 것을 전제로 거래량 분석을 통해 주가를 분석하는 기법으로 조셉 그렌빌(Joseph Granville)이 개발한 기법이다. OBV_EMA 는 OBV의 지수이동평균 선이다./nOBV가 OBV_EMA위로 오는 경우 낮음으로 판단한다.",
    params: {
      1: "OBV, OBV_EMA 기간",
    },
  },
  507: {
    info: "MFI는 거래량 가중 상대적 강도 지수라고도 하며, 자산 중에서 과매수 또는 과매도 신호를 식별하기 위해 가격 및 거래량 데이터를 이용해 계산한다. MFI 값이 80이상이면 과매수(매도 타이밍)로 간주되고, 20 미만이면 과매도(매수 타이밍)로 간주된다. 또한 MFI 값이 90 또는 10인 경우를 임계값 이라고 한다./nMFI가 임계치보다 위에 있다면 과매수로 판단한다.",
    params: {
      1: "MFI 기간",
      6: "과매수 판단 기준",
    },
  ...
  508: {
    info: "MFI는 거래량 가중 상대적 강도 지수라고도 하며, 자산 중에서 과매수 또는 과매도 신호를 식별하기 위해 가격 및 거래량 데이터를 이용해 계산한다. MFI 값이 80이상이면 과매수(매도 타이밍)로 간주되고, 20 미만이면 과매도(매수 타이밍)로 간주된다. 또한 MFI 값이 90 또는 10인 경우를 임계값 이라고 한다./nMFI가 임계치보다 아래에 있다면 과매도로 판단한다.",
    params: {
      1: "MFI 기간",
      6: "과매도 기준",
    },
  ...
  707: {
    info: "코스피지수는 증권거래소에 상장된 주식의 증권시장지표 중에서 주식의 전반적인 동향을 가장 잘 나타내는 대표적인 지수이다. 시장전체의 주가 움직임을 측정하는 지표로 이용되며, 투자성과 측정, 다른 금융상품과의 수익률 비교척도, 경제상황 예측지표로도 이용된다./n코스피지수가 이동평균선+오차 위이면 높음으로 판단한다.",
    params: {
      1: "코스피지수 이동평균선 기간",
      2: "코스피지수 이동평균선 오차 허용 범위",
    },
  },
  708: {
    info: "코스피지수는 증권거래소에 상장된 주식의 증권시장지표 중에서 주식의 전반적인 동향을 가장 잘 나타내는 대표적인 지수이다. 시장전체의 주가 움직임을 측정하는 지표로 이용되며, 투자성과 측정, 다른 금융상품과의 수익률 비교척도, 경제상황 예측지표로도 이용된다./n코스피지수가 이동평균선-오차 아래이면 낮음으로 판단한다.",
    params: {
      1: "코스피지수 이동평균선 기간",
      2: "코스피지수 이동평균선 오차 허용 범위",
    },
  ...
  807: {
    info: "코스닥지수는 코스닥시장 전체의 주가동향을 파악할 수 있는 투자분석지표이다./n코스닥지수가 이동평균선+오차 위이면 높음으로 판단한다.",
    params: {
      1: "코스닥지수 이동평균선 기간",
      2: "코스닥지수 이동평균선 오차 허용 범위",
    },
  },
  808: {
    info: "코스닥지수는 코스닥시장 전체의 주가동향을 파악할 수 있는 투자분석지표이다./n코스닥지수가 이동평균선-오차 아래이면 낮음으로 판단한다.",
    params: {
      1: "코스닥지수 이동평균선 기간",
      2: "코스닥지수 이동평균선 오차 허용 범위",
    },
