...
def buy(account, code, price, percent, date, option):
    ...
    for code_num in account['stocks'].keys():
        cur_price = get_price(code_num, date)
    ...
    print(f'매수알림 : {option}에 의해{code} 주식 {price:,} 가격에 {stock_amount:,}주 매수 == 현재 자산 {current_asset} 총 수익률 {earn_rate}')
    return account
...
def sell(account, code, price, percent, date, option):
    ...
    earn_rate = (current_asset - account['start_price']) / account['start_price']
    earn_rate = round(earn_rate, 3) * 100
    print(f'매도알림 : {option}에 의해{code} 주식 {price:,} 가격에 {stock_amount:,}주 매도 == 현재 자산 {current_asset} 총 수익률 {earn_rate}')
    return account

def get_price(code_number, date):
